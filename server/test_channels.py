from server.channels import channel_join, channel_leave, channel_addowner, channel_removeowner, channel_invite, channel_messages, channel_details, channels_list, channels_listall, channel_create
from server.helper import token_to_user, AccessError, reset_data, is_slackr_admin, check_channel_member, is_owner
from server.auth import auth_register
from server.messages import message_send, message_react, message_remove
import pytest
import sys
sys.path.append("../")
import server_data
...
def test_channel_invite():
    ...
    # START SETUP
    token = auth_register('abcd@email.com', 'pass123', 'john', 'apple')['token']
    first_user_id = token_to_user(token)
    second_token = auth_register('newUser@email.com', 'pass147', 'vicks', 'uwu')['token']
    second_user_id = token_to_user(second_token)
    channel_id_private = channel_create(token, 'privateChannel', 'false')
    ...
    assert check_channel_member(third_user_id, channel_id_private) == True
...
def test_channel_messages():
    ...
    # Calling Channel messages when current user is not in the target channel and target channel is private
...
def test_channels_listall():
    ...
    channel_id_private = channel_create(token, 'NotCool', 'false')
    channel_join(second_token, channel_id_public)
    # END SETUP
...
def test_add_all_slackr_and_admins():
    # adds a user who has slackr admin into the channel

    # set-up
    # create first user with slackr permission
    token = auth_register('abcde@email.com', 'pass123', 'john', 'apple')['token']
    creator_id = token_to_user(token)
    assert is_slackr_admin(creator_id) == True
    ...
    assert check_channel_member(creator_id, channel_id_public) == True
    reset_data()
