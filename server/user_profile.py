...
import server_data
from server.helper import is_valid_token, is_valid_name, token_to_user
...
def user_profiles_uploadphoto(token, img_url, x_start, y_start, x_end, y_end):
    ...
    if (x_start > 0 or y_start > 0):
        raise ValueError("x_start, y_start are not within the dimensions of the image at the URL") 

    # crop length must be withing dimensions of the image
    width, height = img.size
    if (x_end > width or y_end > height):
        raise ValueError("x_end and y_end are not within the dimensions of the image at the URL")  
